...
const TestCompoundLong = artifacts.require("CompoundLonging");
...
contract("TestCompoundLong", (accounts) => {
  ...
  const TOKEN_BORROW = ropstenAddress.Contracts.USDT;
  const C_TOKEN_BORROW = ropstenAddress.Contracts.cUSDT;
  const REPAY_WHALE = process.env.DAI_WHALE; // used to repay interest on borrow
  ...
  let tokenBorrow;
  beforeEach(async () => {
    testCompound = await TestCompoundLong.new(accounts[0]);
    await testCompound.initialize(
      CETH,
  ...
  it("should long", async () => {
    ...
    console.log(`eth: ${snap.eth.div(pow(10, 18))}`);
    console.log(
      `token borrow: ${snap.tokenBorrow.div(pow(10, BORROW_DECIMALS))}`
    );
  });
});
